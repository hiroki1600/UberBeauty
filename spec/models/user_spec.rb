  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録がうまくいかないとき' do
      ...
      it "first_nameが空だと登録できない" do
        @user.first_name = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("First name can't be blank", "First name お名前の(姓)には全角文字で入力して下さい")
      end
      it "first_nameが半角だと登録できない" do
        @user.first_name = 'hiroki'
        @user.valid?
        expect(@user.errors.full_messages).to include("First name お名前の(姓)には全角文字で入力して下さい")
      end
      it "last_nameが空だと登録できない" do
        @user.last_name = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name can't be blank", "Last name お名前の(名)には全角文字で入力して下さい")
      end
      it "last_nameが半角だと登録できない" do
        @user.last_name = 'furukawa'
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name お名前の(名)には全角文字で入力して下さい")
      end
      it "first_name_kanaが空だと登録できない" do
        @user.first_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("First name kana can't be blank", "First name kana お名前カナの(姓)には全角文字で入力して下さい")
      end
      it "first_name_kanaが半角だと登録できない" do
        @user.first_name_kana = 'hiroki'
        @user.valid?
        expect(@user.errors.full_messages).to include("First name kana お名前カナの(姓)には全角文字で入力して下さい")
      end
      it "last_name_kanaが空だと登録できない" do
        @user.last_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name kana can't be blank", "Last name kana お名前カナの(名)には全角文字で入力して下さい")
      end
      it "last_name_kanaが半角だと登録できない" do
        @user.last_name_kana = 'furukawa'
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name kana お名前カナの(名)には全角文字で入力して下さい")
      end
